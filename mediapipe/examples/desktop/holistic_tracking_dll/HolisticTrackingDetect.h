...
//#include "mediapipe/framework/port/realsense2_inc.h"
...
namespace GoogleMediapipeDetect {
	...
	public:
		...
		int DetectImageDirect(int image_width, int image_height, void* image_data ,int* detect_result, void* show_data, bool show_result_image = false);
		int DetectImageBodyInfo(int image_width, int image_height, void* image_data , void* show_data, std::vector<std::vector<std::vector<float>>>& vec_BodykeyPoints, bool show_result_image = false);
	...
	private:
		absl::Status Mediapipe_InitGraph(const char* model_path,
			...
			bool is_need_hand_outputstream = true,
			bool is_need_face_outputstream = true);
		absl::Status Mediapipe_RunMPPGraph_Direct(int image_width, int image_height, void* image_data, int* detect_result, void* show_data, bool show_result_image = false);
		absl::Status Mediapipe_RunMPPGraph_BodyInfo(int image_width, int image_height, void* image_data, void* show_data, std::vector<std::vector<std::vector<float>>>& vec_BodykeyPoints, bool show_result_image = false);
