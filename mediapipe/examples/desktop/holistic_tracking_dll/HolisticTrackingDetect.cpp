...
int GoogleMediapipeDetect::HolisticTrackingDetect::DetectImageDirect(int image_width, int image_height, void* image_data, int* detect_result, void* show_data, bool show_result_image)
{
	if (!m_bIsInit)
		return 0;

	absl::Status run_status = Mediapipe_RunMPPGraph_Direct(image_width, image_height, image_data, detect_result, show_data, show_result_image);
	if (!run_status.ok()) {
		return 0;
	}
	return 1;
...
int GoogleMediapipeDetect::HolisticTrackingDetect::DetectImageBodyInfo(int image_width, int image_height, void* image_data, void* show_data, std::vector<std::vector<std::vector<float>>>& vec_BodykeyPoints, bool show_result_image)
{
	if (!m_bIsInit)
		return 0;

	absl::Status run_status = Mediapipe_RunMPPGraph_BodyInfo(image_width, image_height, image_data, show_data, vec_BodykeyPoints, show_result_image);
	if (!run_status.ok()) {
		return 0;
	}
	return 1;
...
)
{
	...
	if(is_need_video_outputstream)
	{
		...
		//m_pVideoPoller->SetMaxQueueSize(3);
	}
...
)
{
	...
	camera_frame.copyTo(camera_frame_RGB);
	...
	if(m_pVideoPoller != nullptr)
	{
		...
		if(m_pVideoPoller->QueueSize() != 0)
		{
			...
			{
				std::cout << "no next packet" << std::endl;
			...
			if(show_result_image)
			{
				...
				// 显示cv::Mat结果
				memcpy(show_data, output_frame_mat.data, image_width * image_height * 3 * sizeof(uchar));
	...
	std::cout << " start left hand........." << std::endl;
	if(m_pLeftHandLandmarksPoller != nullptr)
	{
		...
		{
			std::cout << " left hand .QueueSize.........." << std::endl;
...
absl::Status GoogleMediapipeDetect::HolisticTrackingDetect::Mediapipe_RunMPPGraph_BodyInfo(
	int image_width,
	int image_height,
	void* image_data,
	void* show_data,
	std::vector<std::vector<std::vector<float>>>& vec_BodykeyPoints,
	bool show_result_image)
{
	// 等待图处于空闲状态
//m_Graph.WaitUntilIdle();
...
/*----- 4 得到结果 -----*/
// 1 视频输出结果帧
	if (m_pVideoPoller == nullptr || m_pPoseLandmarksPoller == nullptr ||  m_pVideoPoller->QueueSize() == 0 || m_pPoseLandmarksPoller->QueueSize() == 0)
	{
		m_pVideoPoller->Reset();
		return absl::InvalidArgumentError("no nullptr or queue size equal to zero");
	}
	...
	{
		mediapipe::Packet videoPacket;
		//std::cout << " image queue: " << m_pVideoPoller->QueueSize() << std::endl;
		if (m_pVideoPoller->QueueSize() != 0)
		{
			if (!m_pVideoPoller->Next(&videoPacket))
			{
	...
	if (m_pPoseLandmarksPoller != nullptr)
	{
		...
		if (m_pPoseLandmarksPoller->QueueSize() != 0)
		{
			if (!m_pPoseLandmarksPoller->Next(&poseeLandmarksPacket))
			{
				std::cout << "no next pose packet" << std::endl;
				return absl::InvalidArgumentError("no next pose packet");
			}
			auto& output_landmarks = poseeLandmarksPacket.Get<mediapipe::NormalizedLandmarkList>();
			for (int i = 0; i < output_landmarks.landmark_size(); ++i)
			{
				const mediapipe::NormalizedLandmark landmark = output_landmarks.landmark(i);
				//double tick_s = (double)cv::getTickCount();
				vec_BodykeyPoints[0][i][0] = 0;
				vec_BodykeyPoints[0][i][1] = landmark.x() * camera_frame.cols;
				vec_BodykeyPoints[0][i][2] = landmark.y() * camera_frame.rows;
				vec_BodykeyPoints[0][i][3] = landmark.z() * camera_frame.cols;
				vec_BodykeyPoints[0][i][4] = landmark.visibility();
				vec_BodykeyPoints[0][i][5] = landmark.presence();
				//std::cout << " pose point: " << vec_BodykeyPoints[0][i][1] << " " << vec_BodykeyPoints[0][i][2] << " " << vec_BodykeyPoints[0][i][3] <<  " " << vec_BodykeyPoints[0][i][5] << std::endl;
			}
		}
	}
	...
	if (m_pLeftHandLandmarksPoller != nullptr)
	{
		...
		if (m_pLeftHandLandmarksPoller->QueueSize() != 0)
		{
			if (m_pLeftHandLandmarksPoller->Next(&leftHandLandmarksPacket))
			{
				...
				{
					const mediapipe::NormalizedLandmark landmark = output_landmarks.landmark(i);
					vec_BodykeyPoints[0][33 + i][0] = 1;
					vec_BodykeyPoints[0][33 + i][1] = landmark.x() * camera_frame.cols;
					vec_BodykeyPoints[0][33 + i][2] = landmark.y() * camera_frame.rows;
					vec_BodykeyPoints[0][33 + i][3] = landmark.z() * camera_frame.cols;
					vec_BodykeyPoints[0][33 + i][4] = landmark.visibility();
					vec_BodykeyPoints[0][33 + i][5] = landmark.presence();
					//std::cout << " left hand point: " << x << " " << y << " " << z << " " << landmark.presence() << std::endl;
	...
	if (m_pRightHandLandmarksPoller != nullptr)
	{
		...
		if (m_pRightHandLandmarksPoller->QueueSize() != 0)
		{
			if (m_pRightHandLandmarksPoller->Next(&rightHandLandmarksPacket))
			{
				...
				{
					const mediapipe::NormalizedLandmark landmark = output_landmarks.landmark(i);
					vec_BodykeyPoints[0][54 + i][0] = 2;
					vec_BodykeyPoints[0][54 + i][1] = landmark.x() * camera_frame.cols;
					vec_BodykeyPoints[0][54 + i][2] = landmark.y() * camera_frame.rows;
					vec_BodykeyPoints[0][54 + i][3] = landmark.z() * camera_frame.cols;
					vec_BodykeyPoints[0][54 + i][4] = landmark.visibility();
					vec_BodykeyPoints[0][54 + i][5] = landmark.presence();
					//std::cout << " right hand point: " << x << " " << y << " " << z <<  " " << landmark.presence() << std::endl;
	...
	if (m_pFaceLandmarksPoller != nullptr)
	{
		...
		if (m_pFaceLandmarksPoller->QueueSize() != 0)
		{
			if (m_pFaceLandmarksPoller->Next(&faceLandmarksPacket))
			{
				...
				{
					const mediapipe::NormalizedLandmark landmark = output_landmarks.landmark(i);
					vec_BodykeyPoints[0][75 + i][0] = 3;
					vec_BodykeyPoints[0][75 + i][1] = landmark.x() * camera_frame.cols;
					vec_BodykeyPoints[0][75 + i][2] = landmark.y() * camera_frame.rows;
					vec_BodykeyPoints[0][75 + i][3] = landmark.z() * camera_frame.cols;
					vec_BodykeyPoints[0][75 + i][4] = landmark.visibility();
					vec_BodykeyPoints[0][75 + i][5] = landmark.presence();
				}
			}
		}
	}
